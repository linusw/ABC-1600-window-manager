 ...
 * set up new initial driver and terminal parameters for the window handler.
 * It supposes file descriptor 3 to be the window handler "super" channel.
 * The syntax is:
 *
 *	widtp [-n] [file]
 *
...
char	**argv;		/* pointer to arguments */
{
	...
	struct	wininit wpd;		/* to set up new parameters */
	...
	if (file != NULL && (fd = open(file, 0)) < 0) {
		error("widtp: cannot open data file\n");
	}
	cnt2 = 0;
	while ((cnt1 = read(fd, (char *)(&wpd), sizeof(struct wininit) - cnt2)) > 0 &&
	       (cnt2 += cnt1) != sizeof(struct wininit))
		;
	if (cnt2 != sizeof(struct wininit)) {
		error("widtp: cannot read data\n");
	}

	/*
	 * Now we are ready to set up new initial driver and terminal
	 * parameters.
	 */
	if (Winsinit(SUPERCH, &wpd) < 0) {
		error("widtp: failed to set up initial parameters\n");
	}
