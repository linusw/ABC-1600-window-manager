 ...
 * This command repeatedly draws a line from 0,0 to 100,100 back and forth.
 */
...
char	**argv;
{
	...
	if (argc <= 1) {
		...
		error("usage: grtest1 [<count>]\n");
	}
	str1 = "\033:0;0m";
	str2 = "\033:100;100;;1d\033:0;0d";
	len = strlen(str1) + count * strlen(str2);
	if ((int)(buf1 = buf2 = sbrk(len)) == -1) {
		error("grtest1: can't allocate memory\n");
	}
	while (*str1 != '\0') {
		*buf1++ = *str1++;
	}
